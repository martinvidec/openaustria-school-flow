...
- **D-11:** Farb-Picker — **[USER-OVERRIDE: Free hex picker war NICHT recommended, User wählte free hex]**. Implementation: native `<input type="color">` oder shadcn-konformer Color-Popover. **Implikation:** Timetable-Cell-Contrast kann leiden — Mitigation: (a) Live-Preview im Dialog mit echtem Timetable-Cell-Mockup (weißer Text auf gewählter Farbe) + WCAG-AA-Warnhinweis ("Kontrast unzureichend für Timetable") bei contrast-ratio < 4.5:1 (berechnet via color-ratio util). Admin kann speichern trotz Warning; Verantwortung beim Admin. (b) Phase-4-SUBJECT_PALETTE-Presets als "Empfohlene Farben"-Swatch-Leiste über dem Picker für 1-Click-Set.
