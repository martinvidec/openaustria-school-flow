...
- [x] Admin UI für DSGVO-Verwaltung (Einwilligungs-Audit, Aufbewahrungs-Editor, DSFA/VVZ, manuelle Art. 15/17-Auslösung) — Validated in Phase 15: DSGVO-Admin & Audit-Log-Viewer (DSGVO-ADM-01..06 + ConsentsTab/RetentionTab/DsfaVvzTab/JobsTab + RequestExportDialog + 2-step RequestDeletionDialog + 7 Playwright spec files)
- [x] Admin UI für Audit-Log-Viewer mit Such- und Filterfunktion — Validated in Phase 15: DSGVO-Admin & Audit-Log-Viewer (AUDIT-VIEW-01..03 + AuditFilterToolbar URL-synced 7-field filter + AuditDetailDrawer Vorzustand/Nachzustand JsonTree + CSV export with UTF-8 BOM/semicolon/10k cap + AuditInterceptor DSGVO_SUB_RESOURCES allowlist patch from 15-12 + 3 Playwright spec files)
...
**Phase 15 (DSGVO-Admin & Audit-Log-Viewer) completed 2026-04-28.** Shipped DSGVO-ADM-01..06 + AUDIT-VIEW-01..03 admin/dsgvo + admin/audit-log surfaces across 12 plans (11 build + 1 gap closure). Backend: AuditEntry `before` column migration + AuditInterceptor refactor for pre-mutation snapshots + `action` filter on QueryAuditDto, `GET /api/v1/audit/export.csv` (UTF-8 BOM + semicolon + 10k cap), tenant-scoped + role-gated `GET /dsgvo/consent/admin` filter endpoint, new `dsgvo/jobs` module exposing `GET /dsgvo/jobs` for school-wide BullMQ job listing. Frontend: 4-tab `/admin/dsgvo` (Einwilligungen + Aufbewahrung + DSFA/VVZ + Jobs) + `/admin/audit-log` viewer with AuditFilterToolbar (7 URL-synced fields), AuditTable, AuditDetailDrawer with Vorzustand/Nachzustand JsonTree, RequestExportDialog (Art. 15), 2-step strict-equal RequestDeletionDialog (Art. 17), polling hooks with terminal-stop pattern. 10 Playwright spec files (7 DSGVO + 3 audit-log) including RBAC-negative case. Plan 15-12 closed the verifier-flagged gap: `AuditInterceptor.extractResource()` now walks past `/api/v1/dsgvo/<sub>/...` for the 7 sub-resources via a `DSGVO_SUB_RESOURCES` allowlist Set, `SENSITIVE_RESOURCES` extended with dsfa/vvz/deletion, E2E helper + detail spec reverted off the `/schools/:id` workaround back to `/dsgvo/retention/:id`. 138/138 audit + dsgvo module tests pass. Deferred: DTO `@IsUUID()` vs seed.ts static-ID alignment (test-fixture only, not a production bug — production uses Prisma `@default(uuid())`); routed to Phase 16 / 15-13 cleanup. Next: Phase 16 (Admin-Dashboard & Mobile-Härtung).

**Phase 16 (Admin-Dashboard & Mobile-Härtung) completed 2026-05-01.** Shipped ADMIN-01..03 + MOBILE-ADM-01..03 across 7 plans (4 waves). Backend: read-only `GET /admin/dashboard/status` aggregating 10 Setup-Completeness categories in 17 reads via `Promise.all` per D-10 single round-trip; admin-only + tenant-scoped via canonical `Person.findFirst({ where: { keycloakUserId } })` pattern; D-23 solver heuristic (configExists union + COMPLETED run gate) + D-24 timegrid heuristic (Period count AND active-SchoolDay count). Plan 16-01 also resolved a Rule-1 deviation: `QueryDashboardDto` accepts seed string school IDs (production still safe — cross-tenant guard sits on `resolveAdminSchoolId`, not the DTO). Frontend: `useIsMobile` extracted to `apps/web/src/hooks/`, `useDashboardStatus` polling hook (D-07/08/09) exporting `dashboardKeys`, shared dual-mode `<DataList>` (table @ ≥sm + cards @ <sm with `getRowAttrs?`), `<ChecklistItem>` + `<DashboardChecklist>` composing 10 items in D-06 order. `/admin/index.tsx` route + role-aware login redirect (D-02: admin → `/admin`, others → `/timetable`) + 6/6 sidebar parity (Dashboard NEW + DSGVO-Verwaltung + Audit-Log added to MobileSidebar). Touch-target lift: shadcn Button/Input/Select/Textarea floor at `min-h-11` on `<sm` per D-17, `sm:min-h-10`/`sm:min-h-9` keep dense desktop. 9 of 11 Phase 14/15 admin tables migrated to DataList for mobile-card alternatives (2 grid-already surfaces explicitly skipped to avoid Pitfall #7 slider density regression). 13 admin-mutation hook files invalidate `dashboardKeys` across 42 mutation `onSuccess` blocks (hybrid pattern D-07; PATTERNS audit prediction of 57 traced to grep-counting import lines, 42 was actual). 4 new Playwright specs (login-redirect + admin-dashboard desktop + admin-dashboard mobile + admin-mobile-sweep mobile) plus a `desktop-firefox` Playwright project for cross-engine login-redirect coverage (Bus-Error-10 hits BOTH desktop- and mobile-WebKit on darwin-arm64 PW 1.59.x — Firefox/Gecko is the only viable third-engine surface). Live-stack run: login-redirect 5/5 chromium + 5/5 Firefox, admin-dashboard 7/7 desktop, admin-dashboard mobile-chrome 5/5 (incl. icon-only badge collapse + drawer focus-management). Bonus: a real Rule-2 a11y gap was discovered + fixed during gap closure — MobileSidebar had ZERO focus-management primitives (no `role=dialog`, no `aria-modal`, no auto-focus, no Escape handler, no return-focus); all four added. 12/12 v1.1 phases complete (100%). Validated in Phase 16: ADMIN-01/02/03 + MOBILE-ADM-01/02/03. Deferred to backlog: 3 Phase 16-04 sweep targets at h-9/h-10 (`command.tsx`, `toggle.tsx`, `tabs.tsx`); 2 pre-Phase-16 admin entries still missing from MobileSidebar (Vertretungsplanung, Meine Vertretungen); admin-mobile-sweep deferred items 999.1.A–G across 4 pre-existing deviation classes. Next: v1.1 milestone wrap-up (`/gsd:complete-milestone`).
...
---
*Last updated: 2026-05-01 after Phase 16 (Admin-Dashboard & Mobile-Härtung) completed. ADMIN-01..03 + MOBILE-ADM-01..03 shipped across 7 plans / 4 waves. Backend: read-only `GET /admin/dashboard/status` aggregating 10 Setup-Completeness categories in 17 reads via `Promise.all` (D-10 single round-trip) — D-23 solver heuristic + D-24 timegrid heuristic. Frontend: `useIsMobile` extracted, `useDashboardStatus` polling hook + `dashboardKeys` export, shared dual-mode `<DataList>` (table @ ≥sm + cards @ <sm with `getRowAttrs?`), `<ChecklistItem>` + `<DashboardChecklist>` composing 10 items in D-06 order; `/admin/index.tsx` + role-aware login redirect (D-02: admin → /admin, others → /timetable); shadcn Button/Input/Select/Textarea touch-target floor at `min-h-11` on <sm; 9/11 Phase 14/15 admin tables migrated to DataList; 13 admin-mutation hooks invalidate dashboardKeys across 42 onSuccess blocks (hybrid pattern D-07); 4 new Playwright specs + `desktop-firefox` project (Bus-Error-10 hits BOTH desktop+mobile WebKit on darwin-arm64 PW 1.59.x — Firefox/Gecko is the viable third-engine surface). Live-stack run: login-redirect 5/5 chromium + 5/5 Firefox, admin-dashboard desktop 7/7, mobile-chrome 5/5 (incl. icon-only badge + drawer focus). Bonus Rule-2 fix: MobileSidebar got `role=dialog` + `aria-modal` + auto-focus + Escape + return-focus (none existed before). 12/12 v1.1 phases complete (100%) — milestone ready for `/gsd:complete-milestone`. Deferred to backlog: 3 sweep targets (command/toggle/tabs), 2 pre-Phase-16 MobileSidebar entries (Vertretungsplanung, Meine Vertretungen), admin-mobile-sweep deferred items 999.1.A–G across 4 pre-existing deviation classes. Next: v1.1 milestone wrap-up.*
