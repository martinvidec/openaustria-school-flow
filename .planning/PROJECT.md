...
---
*Last updated: 2026-04-28 after Phase 15 (DSGVO-Admin & Audit-Log-Viewer) completed. DSGVO-ADM-01..06 + AUDIT-VIEW-01..03 shipped across 12 plans (11 build + 1 gap closure 15-12). 4-tab `/admin/dsgvo` (Einwilligungen + Aufbewahrung + DSFA/VVZ + Jobs) + `/admin/audit-log` viewer with Vorzustand/Nachzustand JsonTree drawer + CSV export (UTF-8 BOM + semicolon + 10k cap); 2-step strict-equal Art. 17 deletion dialog; AuditInterceptor `before` column + DSGVO_SUB_RESOURCES allowlist Set so DSGVO mutations bucket correctly under their sub-resource (consent/retention/dsfa/vvz/export/deletion/jobs); 10 Playwright spec files including RBAC-negative case. 138/138 audit + dsgvo module tests pass. 11/12 v1.1 phases complete (92%). Deferred: DTO `@IsUUID()` vs seed.ts static-ID alignment (test-fixture only, not a production bug) → Phase 16 / 15-13. Next: Phase 16 (Admin-Dashboard & Mobile-Härtung).*
